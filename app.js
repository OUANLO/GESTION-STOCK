...
async function handleAddUser(e) {
    ...
    try {
        // SOLUTION TEMPORAIRE : Afficher un popup avec les instructions
        const message = `
📝 CRÉATION D'UTILISATEUR

Pour créer l'utilisateur "${name}", suivez ces étapes :

1️⃣ Allez sur https://supabase.com → Votre projet
2️⃣ Authentication → Add user → Create new user
3️⃣ Remplissez :
   • Email : ${email}
   • Password : ${tempPassword}
   • ✓ Auto Confirm User
4️⃣ Créez et COPIEZ l'UUID de l'utilisateur

5️⃣ SQL Editor → Nouvelle requête → Exécutez :

INSERT INTO users (id, name, email, role, permissions, first_login)
VALUES (
    'COLLEZ_UUID_ICI',
    '${name}',
    '${email}',
...
---
💡 Les informations ont été copiées dans votre presse-papier !
        `;
        ...
        alert(message);
        console.log('%c📋 INFORMATIONS UTILISATEUR', 'font-size: 20px; font-weight: bold; color: #667eea;');
        console.log('Nom:', name);
        console.log('Email:', email);
        console.log('Mot de passe:', tempPassword);
        console.log('\n%cSQL à exécuter:', 'font-weight: bold; color: #28a745;');
        console.log(`INSERT INTO users (id, name, email, role, permissions, first_login) VALUES ('UUID_ICI', '${name}', '${email}', 'user', ARRAY['stocks'], true);`);

        showAlert('success', 'Instructions affichées ! Consultez aussi la console (F12) pour copier les commandes.');
